...
public class JarRegistry {
    ...
    private final HashSet<String> loadedJars;
    ...
    protected JarRegistry(final SparkContext context) {
        ...
        this.loadedJars = new HashSet<String>();
    }
     ...
     */
    public synchronized void ensureJarsAreLoaded(final List<String> jarFilesToLoad) throws GenericKnimeSparkException {
        try {
            ...
            for (String jarFileToLoad : jarFilesToLoad) {
                if (!loadedJars.contains(jarFileToLoad)) {
                    LOGGER.log(Level.INFO,
                        String.format("Loading jar file into Spark driver and workers: %s", jarFileToLoad));
                    context.addJar(jarFileToLoad);
                    addToClassLoader(new URL("file:" + jarFileToLoad), contextClassLoader);
                    loadedJars.add(jarFileToLoad);
                }
            }
        } catch (Exception e) {
    ...
    public static synchronized JarRegistry getInstance(final SparkContext context) {
        ...
        if (perContextSingleton.context != context) {
