...
public class SparkConcatenateNodeModel extends SparkNodeModel {
    ...
    protected PortObject[] executeInternal(final PortObject[] inData, final ExecutionContext exec) throws Exception {
        ...
        for (PortObject in : inData) {
            if (in instanceof SparkDataPortObject) {
                ...
                if (context == null || spec == null) {
                    context = sparkRDD.getContext();
                    spec = sparkRDD.getTableSpec();
                } else {
                    context.compatible(sparkRDD.getContext());
        ...
        if (rdds.size() == 1) {
            ...
            final ConcatenateRDDsTask task = new ConcatenateRDDsTask(rdds.toArray(new SparkRDD[0]), resultTable.getID());
            task.execute(exec);
