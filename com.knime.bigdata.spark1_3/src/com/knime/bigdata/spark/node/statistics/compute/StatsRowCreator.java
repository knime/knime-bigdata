...
public class StatsRowCreator implements Iterable<DataRow>{
     ...
     * @param settings
     */
    public StatsRowCreator(final BoundedMultiVariateStatisticalSummary stats, final MLlibSettings settings) {
        m_settings = settings;
        m_rowCount = stats.count();
        m_vals[MAX] = stats.max().toArray();
        m_vals[MEAN] = stats.mean().toArray();
        m_vals[MIN] = stats.min().toArray();
        m_vals[L1] = stats.normL1().toArray();
        m_vals[L2] = stats.normL2().toArray();
        m_vals[NONZEROS] = stats.numNonzeros().toArray();
        m_vals[VARIANCE] = stats.variance().toArray();
    }
