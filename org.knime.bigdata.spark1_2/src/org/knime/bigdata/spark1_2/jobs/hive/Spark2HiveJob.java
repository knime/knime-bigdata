...
import org.knime.bigdata.spark1_2.api.TypeConverters;
...
public class Spark2HiveJob implements SimpleSparkJob<Spark2HiveJobInput> {
     ...
     */
    private void runWithParquetcompression(final JavaHiveContext hiveContext, final String sqlStatement,
        ...
        try {
            ...
            try {
                previousCompression = hiveContext.sqlContext().getConf(SPARK_SQL_PARQUET_COMPRESSION_CODEC);
                LOGGER.log(Level.INFO, "Found PARQUET compression {0}.", previousCompression);
            } catch (NoSuchElementException e) {
                LOGGER.log(Level.INFO, "No previous PARQUET compression found.");
                //Nothing to do
            }
            // Set compression
            ...
            hiveContext.sqlContext().setConf(SPARK_SQL_PARQUET_COMPRESSION_CODEC, codecString);
    ...
    private void resetParquetCompression(final JavaHiveContext hiveContext, final String previousCompression) {
        try {
            if (previousCompression.isEmpty()) {
                // There is no unset() in Spark 1.*.
                // Set it back to the default given in the spark documentation
                hiveContext.sqlContext().setConf(SPARK_SQL_PARQUET_COMPRESSION_CODEC,
                    ParquetCompression.GZIP.toString());
            } else {
                hiveContext.sqlContext().setConf(SPARK_SQL_PARQUET_COMPRESSION_CODEC, previousCompression);
