...
public class HDFSLocalRemoteFile extends RemoteFile<HDFSLocalConnection> {
	 ...
	 */
	@Override
	protected HDFSLocalConnection createConnection() {
		return new HDFSLocalConnection(getConnectionInformation());
	}
	...
	private HDFSLocalConnection getOpenedConnection() throws IOException {
		try {
			open();
	 ...
	 */
	@Override
	public HDFSLocalRemoteFile[] listFiles() throws Exception {
		try {
			final FileStatus[] listStatus = getOpenedConnection().listFiles(getURI());
			if (listStatus == null) {
				return new HDFSLocalRemoteFile[0];
			}
			final HDFSLocalRemoteFile[] files = new HDFSLocalRemoteFile[listStatus.length];
			for (int i = 0, length = listStatus.length; i < length; i++) {
				final URI uri = HDFSLocalConnection.getHDFSPath4URI(getConnectionInformation(),
								listStatus[i].getPath().toUri()).toUri();
				files[i] = new HDFSLocalRemoteFile(uri, getConnectionInformation(),
						getConnectionMonitor());
			}
			return files;

		} catch (final org.apache.hadoop.security.AccessControlException e) {
