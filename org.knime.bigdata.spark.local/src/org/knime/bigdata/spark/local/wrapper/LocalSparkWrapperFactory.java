...
public class LocalSparkWrapperFactory {
    ...
    /**
     * Creates a {@link LocalSparkWrapper} that lives in its own class loader hierarchy. The returned object can be used
     * to create/destroy a Spark context in local mode and run jobs on it.
     *
     * @param jobJar The KNIME job jar.
     * @param extraJars Additional jars that should be on the classpath of the custom class loader hierarchy.
     * @return a {@link LocalSparkWrapper}, which is ready to create a Spark context in local mode.
     */
        ...
        return new URLClassLoader(getJars(jobJar, extraJars), bundleDelegatingLoader) {
            ...
            public Class<?> loadClass(final String name) throws ClassNotFoundException {
                ...
                if (name.equals(LocalSparkWrapper.class.getName())
                    || name.equals(LocalFileSystemHiveTempWrapper.class.getName())) {
    ...
    private static URL[] getJars(final File jobJar, final File[] extraJars) throws IOException {
        final File sparkJarDir =
            new File(FileLocator.getBundleFile(FrameworkUtil.getBundle(LocalSparkWrapperFactory.class)), "/libs");
