...
import com.knime.bigdata.spark.node.mllib.reduction.pca.PCATask;
...
public class PCATaskTest extends SparkWithJobServerSpec {
	...
	public static String paramsAsJason(final String aInputTableName,
			final Integer[] aNumericColIdx, final int aK, final String aMatrix) {
	...
	@Test
	public void verifyThatPCAJobStoresResultMatrixAsNamedRDDAndReturnsVector()
			throws Throwable {
		...
		PCATask testObj = new PCATask(context, "tab1", cols, k, "u");

		double[][] v = testObj.execute(null);

		assertEquals("expected num selected columns", cols.length, v.length);
		assertEquals("expected num principal components", k, v[0].length);
