...
public class RDDUtilsInJavaTest extends LocalSparkSpec {
	...
	private static class MyMapper implements Serializable {
		...
		JavaRDD<Row> toRowRddWithNominalLabels(final JavaDoubleRDD o) {
			return o.map(new Function<Double, Row>() {
				...
				public Row call(final Double x) {
					ix = ix + 1;
					final String color = colors[(int) (colors.length * Math
							.random())];
		...
		JavaRDD<String> extractLabelColumn(
				final JavaRDD<Row> aJavaRDDWithNominalLabels) {
	...
	@Test
	public void toLabeledPointRDDShouldCreateVectorsForIndicatedColumnsOnly()
			throws Exception {
		...
		colIdxs.add(1);
		colIdxs.add(3);
		final List<LabeledPoint> inputRdd = RDDUtilsInJava
				.toJavaLabeledPointRDD(rowRDD, colIdxs, 2).collect();

		List<Row> rows = rowRDD.collect();
		assertEquals("number of rows must not change", rows.size(),
				inputRdd.size());

		int ix = 0;
		for (LabeledPoint p : inputRdd) {
			assertEquals(
					"length of feature vector must be equivalent to length of colum selector",
					colIdxs.size(), p.features().size());
			assertEquals("selected features incorrect (col 1 of row[" + ix
					+ "])", rows.get(ix).get(1), p.features().apply(0));
			assertEquals("selected features incorrect (col 3 of row[" + ix
					+ "])", rows.get(ix).get(3), p.features().apply(1));
			ix++;
		}
	}
	...
	@Test
	public void conversionOfJavaRowRDD2RowMatrix() throws Exception {
		final int size = 100;
		JavaDoubleRDD o = getRandomDoubleRDD(size, 2);
		JavaRDD<Row> rowRdd = new MyMapper().toRowRdd(o);
		List<Integer> indices = new ArrayList<>();
		indices.add(0);
		indices.add(1);
		indices.add(2);
		indices.add(3);
		RowMatrix matrix = RDDUtilsInJava.toRowMatrix(rowRdd, indices);

		RDD<Vector> rows = matrix.rows();
		assertEquals("conversion should keep number rows constant ",
				rows.count(), size);
		Object[] vectors = (Object[]) rows.collect();
		List<Row> expected = rowRdd.collect();
		for (int i = 0; i < vectors.length; i++) {
			Vector v = (Vector) vectors[i];
			assertEquals("conversion should create correct length of vectors ",
					4, v.size());
			Row ex = expected.get(i);
			for (int r = 0; r < ex.length(); r++) {
				assertEquals("conversion should not change values ",
						((Number) ex.get(r)).doubleValue(), v.apply(r),
						0.0000001);
			}
		}
	}
	...
	@Test
	public void conversionOfJavaRowRDD2RowMatrixWithColumnSelection()
			throws Exception {
		final int size = 15;
		JavaDoubleRDD o = getRandomDoubleRDD(size, 2);
		JavaRDD<Row> rowRdd = new MyMapper().toRowRdd(o);
		...
		indices.add(2);
		RowMatrix matrix = RDDUtilsInJava.toRowMatrix(rowRdd, indices);
		final List<Row> expected = rowRdd.collect();

		{
			...
			for (int i = 0; i < vectors.length; i++) {
				Vector v = (Vector) vectors[i];
				assertEquals(
						"conversion should create correct length of vectors ",
						2, v.size());
				Row ex = expected.get(i);
				assertEquals("conversion should not change values ",
						((Number) ex.get(1)).doubleValue(), v.apply(0),
						0.0000001);
				assertEquals("conversion should not change values ",
						((Number) ex.get(2)).doubleValue(), v.apply(1),
						0.0000001);
			}
		}
		{
			JavaRDD<Row> covertedBackToRDD = RDDUtilsInJava
					.fromRowMatrix(matrix);
			assertEquals("re-conversion should keep number rows constant ",
					covertedBackToRDD.count(), size);
			List<Row> rowList = covertedBackToRDD.collect();
			for (int i = 0; i < rowList.size(); i++) {
				Row v = rowList.get(i);
				assertEquals(
						"re-conversion should create correct length of vectors ",
						2, v.length());
				Row ex = expected.get(i);
				assertEquals("re-conversion should not change values ",
						((Number) ex.get(1)).doubleValue(), v.getDouble(0),
						0.0000001);
				assertEquals("re-conversion should not change values ",
						((Number) ex.get(2)).doubleValue(), v.getDouble(1),
						0.0000001);
			}
		}
	...
	@Test
	public void conversionOfMatrix2JavaRDDOfRows() throws Exception {
	...
	@Test
	public void conversionOfJavaRowRDDWithNominalValues2JavaRDDWithLabeledPoint()
			throws Exception {
		JavaDoubleRDD o = getRandomDoubleRDD(100L, 1);
		...
		selector.add(1);
		selector.add(2);
		LabeledDataInfo info = RDDUtilsInJava
				.toJavaLabeledPointRDDConvertNominalValues(v, selector, 3);

		assertEquals("Incorrect number of classes", 5, info
				.getClassLabelToIntMapping().size());

		JavaRDD<LabeledPoint> rowRDD = info.getLabeledPointRDD();
		assertEquals("Conversion changed the number of rows ", rowRDD.count(),
				100);
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			assertEquals(
					"conversion should set proper label ",
					info.getClassLabelToIntMapping()
							.getNumberForValue(3, labels.get(i)).intValue(),
					(int) rows.get(i).label());
		}
	}

	/**
	 * convert all nominal values in selected columns to corresponding columns
	 * with numbers (not binary), use one mapping for all columns
	 *
	 * @throws Exception
	 */
	@Test
	public void conversionOfNominalValuesInRDDOfRowsOneMap() throws Exception {
		...
		assertEquals("Conversion changed the number of rows ",
				rddWithConvertedValues.count(), 10);
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			assertTrue(
					"converted values should be numbers and at the end",
					mappings.getNumberForValue(0, row.getString(0)) == (int) row
							.getDouble(6));
			assertTrue(
					"converted values should be numbers and at the end",
					mappings.getNumberForValue(2, row.getString(2)) == (int) row
							.getDouble(7));
			assertTrue(
					"converted values should be numbers and at the end",
					mappings.getNumberForValue(3, row.getString(3)) == (int) row
							.getDouble(8));
			assertTrue(
					"converted values should be numbers and at the end",
					mappings.getNumberForValue(4, row.getString(4)) == (int) row
							.getDouble(9));
		}
	}
	...
	public void applyLabelMappingShouldIgnoreUnknownColumns() throws Exception {
		JavaDoubleRDD o = getRandomDoubleRDD(10L, 1);
		// v has 6 columns: team, x, team + color, team, color.substring(0, 1),
		// color
		JavaRDD<Row> v = new MyMapper().toRowRddWithNominalValues(o).cache();
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			assertTrue(
					"converted values should be numbers and at the end",
					info.m_Mappings.getNumberForValue(0, row.getString(0)) == (int) row
							.getDouble(6));
			assertTrue(
					"converted values should be numbers and at the end",
					info.m_Mappings.getNumberForValue(2, row.getString(2)) == (int) row
							.getDouble(7));
		}
	}

	@Test(expected = SparkException.class)
	public void applyLabelMappingShouldReportUnknownValuesInKnownColumns()
			throws Throwable {
		JavaDoubleRDD o = getRandomDoubleRDD(10L, 1);
		// v has 6 columns: team, x, team + color, team, color.substring(0, 1),
		// color
	...
	/**
	 * convert all nominal values in selected columns to corresponding columns
	 * with numbers (not binary), use separate mappings for each column
	 *
	 * @throws Exception
	 */
	@Test
	public void conversionOfNominalValuesInRDDOfRowsSeparateMaps()
			throws Exception {
	...
	public void conversionOfNominalValuesInRDDOfRowsBinary() throws Exception {
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			// assertEquals("incorrect number of values for 'colors'",
			// MyMapper.colors.length,
			// mappings.getNumberOfValues(4));
			// assertTrue("converted values should be in proper column", colIx
			// >= 0
			// && colIx < 4);
			// assertEquals("converted values should be 1", 1,
			// (int)row.getDouble(colIx + offset));
			// offset += mappings.getNumberOfValues(4);
			// }
			assertEquals("incorrect number of new columns added ", 6 + 32,
					offset);
	...
	static class CreateData implements Serializable {
		...
		NormalizedRDDContainer normalize(final Collection<Integer> indices,
				final NormalizationSettings aSettings) {
	...
	public void findColumnStatsShouldComputeMinMaxValues() throws Throwable {
		...
		MultivariateStatisticalSummary stats = RDDUtilsInJava.findColumnStats(
				data.rowData, colIds);
	...
	@Test
	public void minMaxComputationShouldNormalizeDataTo01Range()
			throws Throwable {
		...
		colIds.add(1);
		colIds.add(0);
		JavaRDD<Row> normalizedData = data.normalize(
				colIds,
				NormalizationSettingsFactory
	...
	@Test
	public void minMaxComputationShouldNormalizeDataToXYRange()
			throws Throwable {
		CreateData data = new CreateData(sparkContextResource.sparkContext);
		...
		colIds.add(1);
		colIds.add(0);
		NormalizedRDDContainer normalizedData = data.normalize(colIds,
				NormalizationSettingsFactory
						.createNormalizationSettingsForMinMaxScaling(-1, 3));

		MultivariateStatisticalSummary stats = RDDUtilsInJava.findColumnStats(
				data.rowData, colIds);

		assertEquals("sepal length", -1d,
				normalizedData.normalize(0, stats.min().apply(0)), 0.0001);
		assertEquals("sepal width", -1d,
				normalizedData.normalize(1, stats.min().apply(1)), 0.0001);
		assertEquals("petal width", -1d,
				normalizedData.normalize(2, stats.min().apply(2)), 0.0001);
		assertEquals("sepal length", 3d,
				normalizedData.normalize(0, stats.max().apply(0)), 0.0001);
		assertEquals("sepal width", 3d,
				normalizedData.normalize(1, stats.max().apply(1)), 0.0001);
		assertEquals("petal width", 3d,
				normalizedData.normalize(2, stats.max().apply(2)), 0.0001);

		// new range is 4, middle value should be normalized to 1
		assertEquals(
				"sepal width normalized mean value",
				1,
				normalizedData.normalize(1, stats.min().apply(1)
						+ (stats.max().apply(1) - stats.min().apply(1)) * 0.5),
				0.0001);
		...
		for (double d = stats.min().apply(0); d < stats.max().apply(0); d = d + 0.1d) {
			assertEquals("sepal length normalized value", -1d + scale
					* (d - stats.min().apply(0)),
		...
		colIds.add(2);
		colIds.add(3);
		NormalizedRDDContainer minMax = RDDUtilsInJava.normalize(normalizedData
				.getRdd(), colIds, NormalizationSettingsFactory
				.createNormalizationSettingsForMinMaxScaling(0, 1));

		stats = RDDUtilsInJava.findColumnStats(normalizedData.getRdd(), colIds);

		// the normalized columns should have min / max value -1/3, the
		// unnormalized column the original min/max
		assertEquals("petal length unnormalized", -1d, stats.min().apply(0),
				0.0001);
		assertEquals("petal length unnormalized", 3d, stats.max().apply(0),
				0.0001);
		assertEquals("petal length unnormalized", -1d, stats.min().apply(1),
				0.0001);
		assertEquals("petal length unnormalized", 3d, stats.max().apply(1),
				0.0001);
		assertEquals("petal length unnormalized", 1d, stats.min().apply(2),
				0.0001);
		assertEquals("petal length unnormalized", 6.9d, stats.max().apply(2),
				0.0001);
		assertEquals("petal length unnormalized", -1d, stats.min().apply(3),
				0.0001);
		assertEquals("petal length unnormalized", 3d, stats.max().apply(3),
				0.0001);

		assertEquals("sepal length", 0d,
				minMax.normalize(0, stats.min().apply(0)), 0.0001);
		assertEquals("sepal width", 0d,
				minMax.normalize(1, stats.min().apply(1)), 0.0001);
		assertEquals("petal length unnormalized", 0d,
				minMax.normalize(2, stats.min().apply(2)), 0.0001);
		assertEquals("petal width", 0.0d,
				minMax.normalize(3, stats.min().apply(3)), 0.0001);
		assertEquals("sepal length", 1d,
				minMax.normalize(0, stats.max().apply(0)), 0.0001);
		assertEquals("sepal width", 1d,
				minMax.normalize(1, stats.max().apply(1)), 0.0001);
		assertEquals("petal length unnormalized", 1d,
				minMax.normalize(2, stats.max().apply(2)), 0.0001);
		assertEquals("petal width", 1d,
				minMax.normalize(3, stats.max().apply(3)), 0.0001);
	}
	...
	public void normalizeDataUsingZScore() throws Throwable {
		...
		NormalizedRDDContainer normalizedData = data.normalize(colIds,
				NormalizationSettingsFactory
						.createNormalizationSettingsForZScoreNormalization());

		MultivariateStatisticalSummary stats0 = RDDUtilsInJava.findColumnStats(
				data.rowData, colIds);

		// original mean value should be normalized to 0
		assertEquals("sepal length", 0,
				normalizedData.normalize(0, stats0.mean().apply(0)), 0.0001);
		assertEquals("sepal width", 0,
				normalizedData.normalize(1, stats0.mean().apply(1)), 0.0001);
		assertEquals("petal width", 0,
				normalizedData.normalize(2, stats0.mean().apply(2)), 0.0001);
		// assertEquals("sepal length", 0,normalizedData.normalize(0,
		// normalizedData.getMean(0)), 0.0001);
		// assertEquals("sepal width", 0,normalizedData.normalize(0,
		// normalizedData.getMean(0)), 0.0001);
		// assertEquals("petal width", 0,normalizedData.normalize(0,
		// normalizedData.getMean(0)), 0.0001);

		// compute stats for normalized data, mean should be 0, variance 1
		MultivariateStatisticalSummary stats1 = RDDUtilsInJava.findColumnStats(
				normalizedData.getRdd(), colIds);

		// data is normalized to mean 0 and std dev 1, middle value should be
		// normalized to 0
		assertEquals("sepal length normalized mean value", 0, stats1.mean()
				.apply(0), 0.0000001);
		assertEquals("sepal width normalized mean value", 0, stats1.mean()
				.apply(1), 0.0000001);
		assertEquals("petal width normalized mean value", 0, stats1.mean()
				.apply(2), 0.0000001);

		// square root of 1 is 1
		assertEquals("sepal length normalized standard deviation", 1, stats1
				.variance().apply(0), 0.000001);
		assertEquals("sepal width normalized standard deviation", 1, stats1
				.variance().apply(1), 0.000001);
		assertEquals("petal width normalized standard deviation", 1, stats1
				.variance().apply(2), 0.000001);

		// check values inbetween
		final double mean = stats0.mean().apply(0);
		for (double d = stats0.min().apply(0); d < stats0.max().apply(0); d = d + 0.1d) {
			assertEquals("sepal length normalize value " + d + " for mean: "
					+ mean + ": " + normalizedData.normalize(0, d), d < mean,
					0 > normalizedData.normalize(0, d));
		}
	...
	public void normalizeDataUsingDecimalScaling() throws Throwable {
		...
		colIds.add(1);
		colIds.add(0);
		NormalizedRDDContainer normalizedInfo = new CreateData(
				sparkContextResource.sparkContext).normalize(colIds,
				NormalizationSettingsFactory
						.createNormalizationSettingsForDecimalScaling());

		List<Row> normalizedData = normalizedInfo.getRdd().collect();
		// all values should be divided by: Col 0 - 1000, Col 1 - 10, Col 2 - no
		// normalization, Col 3 - 10
		int ix = 0;
		for (Double[] row : getData()) {
			Row normalizedRow = normalizedData.get(ix++);
			assertEquals("sepal length", row[0] / 1000d,
					normalizedRow.getDouble(0), 0.0001);
			assertEquals("sepal width", row[1] / 10d,
					normalizedRow.getDouble(1), 0.0001);
			// not normalized
			assertEquals("petal length", row[2], normalizedRow.getDouble(2),
					0.0001);
	...
	private final static Double[][] modifiedIrisData = new Double[][] {
			...
			{ 7.0, 3.2, 4.7, 0.4 }, { 6.4, 3.2, 4.5, 0.5 },
			{ 6.9, 3.1, 4.9, 0.5 }, { 5.5, 2.3, 4.0, 0.3 },
			{ 6.5, 2.8, 4.6, 0.5 }, { 5.7, 2.8, 4.5, 0.3 },
			{ 6.3, 3.3, 4.7, 0.6 }, { 4.9, 2.4, 3.3, 0.0 },
			{ 6.6, 2.9, 4.6, 0.3 }, { 5.2, 2.7, 3.9, 0.4 },
			{ 5.0, 2.0, 3.5, 0.0 }, { 5.9, 3.0, 4.2, 0.5 },
			{ 6.0, 2.2, 4.0, 0.0 }, { 6.1, 2.9, 4.7, 0.4 },
			{ 5.6, 2.9, 3.6, 0.3 }, { 6.7, 3.1, 4.4, 0.4 },
			{ 5.6, 3.0, 4.5, 0.5 }, { 5.8, 2.7, 4.1, 0.0 },
			{ -6.2, -2.2, 4.5, 0.5 }, { 5.6, 2.5, 3.9, 0.1 },
			{ 5.9, 3.2, 4.8, 0.8 }, { 6.1, 2.8, 4.0, 0.3 },
			{ 6.3, 2.5, 4.9, 0.5 }, { 6.1, 2.8, 4.7, 0.2 },
			{ 6.4, 2.9, 4.3, 0.3 }, { 6.6, 3.0, 4.4, 0.4 },
			{ 6.8, 2.8, 4.8, 0.4 }, { 6.7, 3.0, 5.0, 0.7 },
			{ 6.0, 2.9, 4.5, 0.5 }, { 5.7, 2.6, 3.5, 0.0 },
			{ 5.5, 2.4, 3.8, 0.1 }, { 5.5, 2.4, 3.7, 0.0 },
			{ 5.8, 2.7, 3.9, 0.2 }, { 6.0, 2.7, 5.1, 0.6 },
			{ 5.4, 3.0, 4.5, 0.5 }, { 6.0, 3.4, 4.5, 0.6 },
			{ 6.7, 3.1, 4.7, 0.5 }, { 6.3, 2.3, 4.4, 0.3 },
			{ 5.6, -3.0, 4.1, 0.3 }, { 5.5, 2.5, 4.0, 0.3 },
			{ 5.5, 2.6, 4.4, 0.2 }, { 6.1, 3.0, 4.6, 0.4 },
			{ 5.8, 2.6, 4.0, 0.2 }, { 5.0, 2.3, 3.3, 0.0 },
			{ 5.6, 2.7, 4.2, 0.3 }, { 5.7, 3.0, 4.2, 0.2 },
			{ 5.7, 2.9, 4.2, 0.3 }, { 6.2, 2.9, 4.3, 0.3 },
			{ 5.1, 2.5, 3.0, 0.1 }, { 5.7, 2.8, 4.1, 0.3 },
			{ 6.3, 3.3, 6.0, 0.5 }, { 5.8, 2.7, 5.1, 0.9 },
			{ 7.1, 3.0, 5.9, 0.1 }, { 6.3, 2.9, 5.6, 0.8 },
			{ 6.5, 3.0, 5.8, 0.2 }, { 7.6, 3.0, 6.6, 0.1 },
			{ 4.9, 2.5, 4.5, 0.7 }, { 7.3, 2.9, 6.3, 0.8 },
			{ 6.7, 2.5, 5.8, 0.8 }, { 7.2, 3.6, 6.1, 0.5 },
			{ 6.5, 3.2, 5.1, 0.0 }, { 6.4, 2.7, 5.3, 0.9 },
			{ 6.8, 3.0, 5.5, 0.1 }, { 5.7, -2.5, 5.0, 0.0 },
			{ 5.8, 2.8, 5.1, 0.4 }, { 6.4, 3.2, 5.3, 0.3 },
			{ 6.5, 3.0, 5.5, 0.8 }, { 7.7, 3.8, 6.7, 0.2 },
			{ 7.7, 2.6, 6.9, 0.3 }, { 6.0, 2.2, 5.0, 0.5 },
			{ 6.9, 3.2, 5.7, 0.3 }, { 5.6, 2.8, 4.9, 0.0 },
			{ 7.7, 2.8, 6.7, 0.0 }, { 176.3, 0.7, 4.9, 0.8 },
			{ 6.7, 3.3, 5.7, 0.1 }, { 7.2, 3.2, 6.0, 0.8 },
			{ 6.2, 2.8, 4.8, 0.8 }, { 6.1, 3.0, 4.9, 0.8 },
			{ 86.4, 2.8, 5.6, 0.1 }, { 7.2, 3.0, 5.8, 0.6 },
			{ 7.4, 2.8, 6.1, 0.9 }, { 7.9, 3.8, 6.4, 0.0 },
			{ 6.4, 2.8, 5.6, 0.2 }, { -666.3, 2.8, 5.1, 0.5 },
			{ 6.1, 2.6, 5.6, 0.4 }, { 7.7, 3.0, 6.1, 0.3 },
			{ 6.3, 3.4, 5.6, 0.4 }, { 6.4, 3.1, 5.5, 0.8 },
			{ 6.0, 3.0, 4.8, 0.8 }, { 6.9, 3.1, 5.4, 0.1 },
			{ 6.7, 3.1, 5.6, 0.4 }, { 6.9, 3.1, 5.1, 0.3 },
			{ 5.8, 2.7, 5.1, 0.9 }, { 6.8, 3.2, 5.9, 0.3 },
			{ 6.7, 3.3, 5.7, 0.5 }, { 6.7, 3.0, 5.2, 0.3 },
			{ 6.3, 0.5, 5.0, 0.9 }, { 6.5, 3.0, 5.2, 0.0 },
			{ 6.2, 3.4, 5.4, 0.3 }, { 5.9, 3.0, 5.1, 0.8 } };
