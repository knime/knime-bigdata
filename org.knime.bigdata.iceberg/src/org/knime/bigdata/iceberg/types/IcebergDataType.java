...
/**
 * External source data type that encapsulates Deltas {@link Type}.
 *
 * Based on {@code KnimeType} of the Big Data Reader.
 *
 * @author Sascha Wolke, KNIME GmbH, Berlin, Germany
 */
public interface IcebergDataType {
    ...
    static IcebergDataType toExternalType(final String serializedType) {
        if (serializedType.startsWith("list(")) {
            return IcebergListType.toExternalType(serializedType);
        } else {
            return IcebergPrimitiveType.toExternalType(serializedType);
        }
    }
