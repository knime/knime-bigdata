...
/**
 * <code>NodeDialog</code> for the "OrcWriter" Node.
 *
 * @author Mareike Hoeger, KNIME GmbH, Konstanz, Germany
 */
public class OrcWriterNodeDialog extends NodeDialogPane {
     ...
     */
    protected OrcWriterNodeDialog() {
        m_filePanel = new RemoteFileChooserPanel(this.getPanel(), "", false, "targetHistory",
                RemoteFileChooser.SELECT_FILE_OR_DIR,
                createFlowVariableModel(OrcWriterNodeSettings.CFGKEY_FILE, FlowVariable.Type.STRING),
        ...
        panel.add(new DialogComponentBoolean(m_settings.getRowKeyModel(), "Write Row Key").getComponentPanel(), gbc);

        ++gbc.gridy;
        panel.add(new DialogComponentBoolean(m_settings.getfileOverwritePolicyModel(), "Overwrite").getComponentPanel(),
                gbc);

        ++gbc.gridy;
        panel.add(new DialogComponentStringSelection(m_settings.getCompressionModel(), "File Compression: ",
                m_settings.getCompressionList()).getComponentPanel(), gbc);

        addTab("Options", panel);

        final JPanel advancedPanel = new JPanel(new GridBagLayout());
        final DialogComponentNumberEdit batchSize = new DialogComponentNumberEdit(m_settings.getBatchSizeModel(),
                "Number of rows in batch: ");
        final DialogComponentNumberEdit stripSize = new DialogComponentNumberEdit(m_settings.getStripSizeModel(),
                "Strip size: ");
        ...
        advancedPanel.add(batchSize.getComponentPanel(), gbc);
        ++gbc.gridy;
        advancedPanel.add(stripSize.getComponentPanel(), gbc);
        ++gbc.gridy;
        advancedPanel.add(numOfLocalChunks.getComponentPanel(), gbc);
