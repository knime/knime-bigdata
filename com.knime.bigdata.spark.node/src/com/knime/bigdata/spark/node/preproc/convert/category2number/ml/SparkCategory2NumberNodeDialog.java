...
class SparkCategory2NumberNodeDialog extends NodeDialogPane {
    ...
    private final DialogComponentStringSelection m_mappingType;
    ...
    private final DialogComponentBoolean m_dropLastValue = new DialogComponentBoolean(
        SparkCategory2NumberNodeModel.createDropLastValueModel(), "Drop last value");

    SparkCategory2NumberNodeDialog() {


        final Collection<String> MAPPING_NAMES = new HashSet<>();
        MAPPING_NAMES.add(MappingType.COLUMN.name());
        MAPPING_NAMES.add(MappingType.BINARY.name());

        m_mappingType = new DialogComponentStringSelection(
            m_mappingTypeModel, "Mapping type: ", MAPPING_NAMES, false);

        /* Enable 'dropLastValue' only if mapping type is binary */
        m_mappingTypeModel.addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(final ChangeEvent e) {
                String selected = ((SettingsModelString)e.getSource())
                        .getStringValue();
                m_dropLastValue.getModel().setEnabled(MappingType.BINARY.name().equals(selected));
            }
        });
        m_dropLastValue.getModel().setEnabled(false);
        ...
        gc.gridx++;
        panel.add(m_dropLastValue.getComponentPanel(), gc);
    ...
    protected void loadSettingsFrom(final NodeSettingsRO settings, final PortObjectSpec[] specs) throws NotConfigurableException {
        ...
        m_dropLastValue.loadSettingsFrom(settings, fakeSpec);
    }
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings) throws InvalidSettingsException {
        ...
        m_dropLastValue.saveSettingsTo(settings);
    }
