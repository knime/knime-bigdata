 ...
 */
public abstract class AbstractSparkJavaSnippetBaseNodeModel extends SparkNodeModel {
    ...
    protected PortObjectSpec[] configureInternal(final PortObjectSpec[] inSpecs) throws InvalidSettingsException {
        ...
        if (!getHelperRegistry().supportsVersion(sparkVersion)) {
            throw new InvalidSettingsException("Unsupported Spark version: " + sparkVersion);
        }

        if (m_loadedSettings != null) {
            m_sparkJavaSnippet = new SparkJSnippet(sparkVersion, m_snippetType, m_loadedSettings, getHelperRegistry());
            ...
            m_sparkJavaSnippet = new SparkJSnippet(sparkVersion, m_snippetType, getHelperRegistry());
        } else {
     ...
     * @param exec
     * @param jobId id of job to run
     * @return an array containing zero or one {@link PortObject}, depending on whether is true or not
     * @throws Exception
     */
    protected PortObject[] executeSnippetJob(final PortObject[] inData, final boolean createOutputObject, final ExecutionContext exec,
    ...
    private Pair<String, File> compileSnippetCached(final SparkVersion sparkVersion)
        ...
        if (!m_compilationCache.isCached(classHash)) {
            ...
            final JavaSnippetHelper helper = getHelperRegistry().getHelper(sparkVersion);
