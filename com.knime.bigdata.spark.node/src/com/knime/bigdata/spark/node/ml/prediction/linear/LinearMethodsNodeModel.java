...
public class LinearMethodsNodeModel extends SparkNodeModel {
    ...
    protected PortObject[] executeInternal(final PortObject[] inObjects, final ExecutionContext exec) throws Exception {
        ...
        exec.checkCanceled();
        final MLSettings s = m_settings.getSettings(data);
        final LinearLearnerJobInput jobInput;
        //FIXME - TODO - there must be a better way...
        int pos = 0;
        NominalFeatureInfo nominalFeatureInfo = new NominalFeatureInfo();
        for(int ix : s.getFeatueColIdxs() ) {
            final DataColumnSpec tmp = data.getTableSpec().getColumnSpec(ix);
            if (tmp.getType().toString().contains("String")) {
                nominalFeatureInfo.add(pos, -1);
            }
            pos++;
        }

            jobInput = new LinearLearnerJobInput(data.getData().getID(), s.getFeatueColIdxs(), nominalFeatureInfo , s.getClassColIdx(),
                m_settings.getNoOfIterations(), m_settings.getRegularization(), m_settings.getSolverType(),
                m_settings.getElasticNetParam(), m_settings.getAddIntercept(), m_settings.getUseFeatureScaling(),
                m_settings.getTolerance());

        final ModelJobOutput result = runFactory.createRun(jobInput).run(data.getContextID(), exec);
        return new PortObject[]{createSparkModelPortObject(data, m_modelName, s.getLearningTableSpec(), s.getClassColName(),
            s.getFatureColNames(), result)};
