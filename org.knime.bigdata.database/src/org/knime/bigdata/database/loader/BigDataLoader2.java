 ...
 */
public class BigDataLoader2 implements DBLoader {

    private static final NodeLogger LOGGER = NodeLogger.getLogger(BigDataLoader2.class);
     ...
     */
    public BigDataLoader2(final DBSessionReference sessionReference) {
    ...
    public void load(final ExecutionMonitor exec, final Object parameters) throws Exception {
        ...
        final BigDataLoaderParameters2 hiveParameters = loadParameters.getAdditionalSettings()
        ...
        insertIntoTable(exec, loadParameters, tmpTable, hiveParameters.getSelectOrderColumnNames(),
            hiveParameters.getPartitionColumns());
    }
     ...
     */
    private String createTempTableAdditionalSQLStatement() {
        if (Impala.DB_TYPE.equals(getSession().getDBType())) {
    ...
    private void insertIntoTable(final ExecutionMonitor exec,
        ...
        //check if we need to use the load local command or not. Local is required when working with local files
        //e.g. in the Create Local Big Data Environment
        final String loadCommand = filePath.startsWith("file://") ? "LOAD DATA LOCAL" : "LOAD DATA";
        final String loadTableCmd = loadCommand +  " INPATH '" + filePath + "' INTO TABLE " + tableName;
