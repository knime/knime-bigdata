 ...
 */
package org.knime.bigdata.spark2_1.jobs.preproc.sampling;
...
public abstract class AbstractSamplingJob implements SparkJob<SamplingJobInput, SamplingJobOutput> {
    ...
    protected SamplingJobOutput noSplitRequired(final SparkContext context, final SamplingJobInput input,
        ...
        if (input.isSplit()) { // empty second output
            ...
            final Dataset<Row> emptyOutput = spark.createDataFrame(Collections.<Row>emptyList(), inputDataset.schema());
            namedObjects.addDataFrame(input.getNamedOutputObjects().get(1), emptyOutput);
        }
    ...
    private SamplingJobOutput fromTopSampling(final SparkContext context, final SamplingJobInput input,
        ...
        final SparkSession spark = SparkSession.builder().sparkContext(context).getOrCreate();
        final Dataset<Row> inputDataset = namedObjects.getDataFrame(input.getFirstNamedInputObject());
        // Dataset API has no mapPartitionsWithIndex :/
        ...
        final Dataset<Row> samplesDataset = spark.createDataFrame(samples, inputDataset.schema());
        ...
        if (input.isSplit()) {
            ...
            final Dataset<Row> othersDataset = spark.createDataFrame(others, inputDataset.schema());
            namedObjects.addDataFrame(input.getNamedOutputObjects().get(1), othersDataset);
        }
     ...
     */
    protected double getFractionToSample(final SamplingJobInput input, final Dataset<Row> inputRdd) {
