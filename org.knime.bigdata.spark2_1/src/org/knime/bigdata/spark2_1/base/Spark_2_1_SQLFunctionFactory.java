...
/**
 * Factory of Spark 2.1 functions.
 *
 * @author Sascha Wolke, KNIME GmbH
 */
@SparkClass
public class Spark_2_1_SQLFunctionFactory implements SparkSQLFunctionFactory<Column> {
    ...
    public Spark_2_1_SQLFunctionFactory() {}
    ...
    public Column getFunctionColumn(final SparkSQLFunctionJobInput agg) {
        ...
        } else if ("asc_nulls_first".equals(name)) {
            column = asc_nulls_first(columnName);
        } else if ("asc_nulls_last".equals(name)) {
            column = asc_nulls_last(columnName);
        } else if (name.equals("approx_count_distinct") && args == 1) {
            column = approx_count_distinct(columnName);
        } else if (name.equals("approx_count_distinct") && args == 2) {
            column = approx_count_distinct(columnName, (double) getArg(agg, 1));
        ...
        } else if ("desc_nulls_first".equals(name)) {
            column = desc_nulls_first(columnName);
        } else if ("desc_nulls_last".equals(name)) {
            column = desc_nulls_last(columnName);
