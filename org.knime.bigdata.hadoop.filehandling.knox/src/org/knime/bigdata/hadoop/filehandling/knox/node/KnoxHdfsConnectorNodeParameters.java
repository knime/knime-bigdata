    ...
    static final class FileSystemConnectionProvider implements StateProvider<FSConnectionProvider> {
        ...
        public FSConnectionProvider computeState(final NodeParametersInput parametersInput)
            ...
            return () -> { // NOSONAR: Longer lambda acceptable, as it improves readability
                ...
                final var settings = new KnoxHdfsConnectorNodeSettings( //
                    m_urlSupplier.get(), //
                    authParams.m_userPwdAuth.m_userPassword.getUsername(), //
                    authParams.m_userPwdAuth.m_userPassword.getPassword(), //
                    m_workingDirectorySupplier.get(), //
                    m_connectionTimeoutSupplier.get(), //
                    m_receiveTimeoutSupplier.get());
                settings.validateValues();

                try {
                    return new KnoxHdfsFSConnection(settings.toFSConnectionConfig(cred));
