...
class KnoxHdfsConnectorNodeParameters implements NodeParameters {
    ...
    private static final class AuthenticationParameters implements NodeParameters {
        ...
        enum AuthenticationMethod {
            @Label(value = "Username & password", description = "Authenticate with a username and password.")
            USERNAME_PASSWORD
        }
            ...
            public AuthenticationMethod load(final NodeSettingsRO settings) throws InvalidSettingsException {
                ...
                throw new InvalidSettingsException(
                    String.format("Unknown authentication method: '%s'. Possible values: '%s', '%s'", typeString,
                        CFG_KEY_USER_PWD, CFG_KEY_USER_PWD_V2));
            }
    ...
    void validateOnConfigure(final CredentialsProvider credProv) throws InvalidSettingsException {
        ...
        if (StringUtils.isBlank(m_workingDirectory)
            || !m_workingDirectory.startsWith(KnoxHdfsFileSystem.PATH_SEPARATOR)) {
    ...
    void validateUrl() throws InvalidSettingsException {
        if (StringUtils.isBlank(m_url)) {
            throw new InvalidSettingsException("URL required.");
        }

        try {
            final var uri = new URI(m_url);
