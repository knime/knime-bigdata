...
import static org.knime.bigdata.delta.engine.parquet.ParquetFilterUtils.toParquetFilter;
...
public class ParquetFileReader {
    ...
    private FSPath m_rootPath;

    public ParquetFileReader(final Configuration configuration, final FSPath rootPath) {
        m_rootPath = rootPath;
    ...
    public CloseableIterator<ColumnarBatch> read(
        final String path, final StructType schema, final Optional<Predicate> predicate) {
        ...
        return new CloseableIterator<ColumnarBatch>() {
            ...
            public boolean hasNext() {
                ...
                try {
                    ...
                    throw new KernelException("Error reading Parquet file: " + path, ex);
                }
            }
            ...
            private void initParquetReaderIfRequired() {
                if (reader == null) {
                    ...
                    try {
                        ...
                        final Path filePath = PathUtil.getHadoopPath(m_rootPath, configuration, path);
                        ...
                        throw new KernelException("Error reading Parquet file: " + path, e);
                    }
                }
            }
        };
    ...
    public static class BatchReadSupport
        ...
        public BatchReadSupport(final int maxBatchSize, final StructType readSchema) {
        ...
        public RecordMaterializer<Object> prepareForRead(
            final Configuration configuration, final Map<String, String> keyValueMetaData, final MessageType fileSchema,
            final ReadContext readContext) {
            rowRecordCollector = new RowRecordCollector(maxBatchSize, readSchema, fileSchema);
            return rowRecordCollector;
        }

        public ColumnarBatch getDataAsColumnarBatch(final int batchSize) {
         ...
         */
        public void finalizeCurrentRow(final long fileRowIndex) {
    ...
    public static class RowRecordCollector
        ...
        public RowRecordCollector(final int maxBatchSize, final StructType readSchema, final MessageType fileSchema) {
    ...
    private static class ParquetFileReaderWithFooter
        ...
        ParquetFileReaderWithFooter(
            final Path filePath, final Configuration configuration, final ParquetMetadata footer) throws IOException {
