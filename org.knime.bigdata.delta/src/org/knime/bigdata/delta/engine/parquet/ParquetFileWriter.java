...
import static org.apache.parquet.hadoop.ParquetOutputFormat.getLongBlockSize;
...
import static org.knime.bigdata.delta.engine.parquet.ParquetStatsReader.readDataFileStatistics;
...
import org.knime.bigdata.delta.engine.parquet.ParquetColumnWriters.ColumnWriter;
...
public class ParquetFileWriter {
    ...
    public ParquetFileWriter(
            ...
            final Path location,
            final List<Column> statsColumns) {
     ...
     */
    public ParquetFileWriter(final Configuration configuration, final Path destPath) {
        this.configuration = requireNonNull(configuration, "configuration is null");
    ...
    public CloseableIterator<DataFileStatus> write(
        ...
        return new CloseableIterator<DataFileStatus>() {
             ...
             */
            void consumeNextRow(final ParquetWriter<Integer> writer) throws IOException {
             ...
             * for given schema.
             */
            BatchWriteSupport createOrGetWriteSupport(final StructType inputSchema) {
    ...
    private static class BatchWriteSupport extends WriteSupport<Integer> {
        ...
        BatchWriteSupport(
                final StructType inputSchema, // WriteSupport created for this specific schema
                final MessageType parquetSchema) { // Parquet equivalent schema
        ...
        void setColumnVectorWriters(final ColumnWriter[] columnWriters) {
            this.columnWriters =
        ...
        @Override
        public void write(final Integer rowId) {
    ...
    private ParquetWriter<Integer> createWriter(
            final Path filePath,
            final WriteSupport<Integer> writeSupport) throws IOException {
    ...
    private class ParquetRowDataBuilder
            extends ParquetWriter.Builder<Integer, ParquetRowDataBuilder> {
        private final WriteSupport<Integer> writeSupport;

        protected ParquetRowDataBuilder(final Path path, final WriteSupport<Integer> writeSupport) {
            super(path2OutputFile(path, configuration));
        ...
        @Override
        protected WriteSupport<Integer> getWriteSupport(final Configuration conf) {
            return writeSupport;
        }

        private static OutputFile path2OutputFile(final Path hadoopPath, final Configuration configuration) {
            OutputFile wrappedFile;
            try {
                wrappedFile = new OutputFileWrapper(HadoopOutputFile.fromPath(hadoopPath, configuration));
                return wrappedFile;
     ...
     */
    private DataFileStatus constructDataFileStatus(final String path, final StructType dataSchema) {
        try {
