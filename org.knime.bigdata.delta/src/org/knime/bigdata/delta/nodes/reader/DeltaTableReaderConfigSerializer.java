...
enum DeltaTableReaderConfigSerializer implements ConfigSerializer<DeltaTableReaderMultiTableReadConfig>,
    ...
    private final TableSpecConfigSerializer<DeltaTableDataType> m_tableSpecSerializer;

    enum DeltaTableDataTypeSerializer implements NodeSettingsSerializer<DeltaTableDataType> {
        ...
        @Override
        public void save(final DeltaTableDataType externalType, final NodeSettingsWO settings) {
            settings.addString(CFG_TYPE, externalType.toSerializableType());
        }

        @Override
        public DeltaTableDataType load(final NodeSettingsRO settings) throws InvalidSettingsException {
            return DeltaTableDataType.toExternalType(settings.getString(CFG_TYPE));
        }
    ...
    public void loadInModel(final DeltaTableReaderMultiTableReadConfig config, final NodeSettingsRO settings)
        ...
        final var read = NodeParametersUtil.loadParameters(settings, DeltaTableReaderNodeSettings.class);
    ...
    public void saveInModel(final DeltaTableReaderMultiTableReadConfig config, final NodeSettingsWO settings) {
        ...
        NodeParametersUtil.saveParameters(DeltaTableReaderNodeSettings.class, defaultSettings, settings);
    ...
    public void validate(final DeltaTableReaderMultiTableReadConfig config, final NodeSettingsRO settings)
            throws InvalidSettingsException {
        NodeParametersUtil.loadParameters(settings, DeltaTableReaderNodeSettings.class);
        if (settings.containsKey(CFG_TABLE_SPEC_CONFIG)) {
            m_tableSpecSerializer.load(settings.getNodeSettings(CFG_TABLE_SPEC_CONFIG));
        }
    }

    static TableSpecConfigSerializer<DeltaTableDataType> getTableSpecSerializer(final ConfigIDLoader configIdLoader) {
