...
import org.knime.bigdata.fileformats.orc.types.OrcBooleanTypeFactory.OrcBooleanType;
...
public class OrcBooleanTypeFactory implements OrcTypeFactory<OrcBooleanType> {
    ...
    public static class OrcBooleanType extends AbstractOrcType<LongColumnVector> {
        ...
        protected DataCell readValueNonNull(final LongColumnVector columnVector, final int rowInBatchCorrected) {
            final long valueL = columnVector.vector[rowInBatchCorrected];
            boolean cellValue;
            if (valueL == 0) {
                cellValue = false;
            } else if (valueL == 1) {
                ...
                throw new BigDataFileFormatException(String.format("No Boolean value: %d", valueL));
            }
