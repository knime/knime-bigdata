...
/**
 * 
 * Helper class for ORC type mapping registration
 * 
 * @author Mareike Hoeger, KNIME GmbH, Konstanz, Germany
 *
 */
public class ORCRegistrationHelper {
     ...
     */
    public static void registerORCConsumers() {

        final List<ORCCellValueConsumerFactory<?, ?>> primitiveConsumers = new ArrayList<>();

        for (final TypeDescription type : m_stringTypes) {
            final ORCCellValueConsumerFactory<String, BytesColumnVector> stringStringKindConsumer = 
        ...
        for (final TypeDescription type : m_intTypes) {
            final ORCCellValueConsumerFactory<Integer, LongColumnVector> integerIntegerConsumer = 
                    new ORCCellValueConsumerFactory<>(
        ...
        for (final TypeDescription type : m_doubleTypes) {
            final ORCCellValueConsumerFactory<Double, DoubleColumnVector> doubleDoubleConsumer = 
                    new ORCCellValueConsumerFactory<>(
                        ...
                        String.class, TypeDescription.createDecimal(), (cv, rowindex, v) -> {
                            ...
                            if (writeable.isSet()) {
                                ...
                                throw new BigDataFileFormatException(
                                        String.format("Could not convert %s to decimal", v));
                            }

                        });
        primitiveConsumers.add(stringBigDecimalConsumer);

        final ORCCellValueConsumerFactory<Byte, LongColumnVector> byteByteConsumer = 
                new ORCCellValueConsumerFactory<>(Byte.class,
                        TypeDescription.createByte(), (cv, rowindex, v) ->
                cv.vector[rowindex] = v.longValue());
        primitiveConsumers.add(byteByteConsumer);

        final ORCCellValueConsumerFactory<Long, LongColumnVector> longLongConsumer = 
                new ORCCellValueConsumerFactory<>(Long.class,
    ...
    /**
     * registers the ORC Producers
     */
    public static void registerORCProducers() {
        ...
        final ORCCellValueProducerFactory<LocalDateTime, TimestampColumnVector> timestampTimestampProducer = 
                new ORCCellValueProducerFactory<>(
                        ...
                        columnVector.asScratchTimestamp(rowInBatchOrZero).toLocalDateTime());
        ...
        final ORCCellValueProducerFactory<String, DecimalColumnVector> bigDecimalDecimalProducer = 
                new ORCCellValueProducerFactory<>(
                        ...
                        columnVector.vector[rowInBatchOrZero].getHiveDecimal().bigDecimalValue().toString());
        ...
        for (final TypeDescription type : m_stringTypes) {
            final ORCCellValueProducerFactory<String, BytesColumnVector> stringStringKindProducer = 
                    new ORCCellValueProducerFactory<>(
                            ...
                            new String(columnVector.vector[rowInBatchOrZero], 
