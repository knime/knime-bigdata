...
class ParquetWriter3NodeParameters implements NodeParameters {
    ...
    @Effect(predicate = IsFileSelection.class, type = EffectType.HIDE)
    ...
    private static final class LegacyFileWriterWithFilterModeAndOverwritePolicyOptions extends //
        ...
        @Widget(title = "Filter mode", description = "false = file mode, true = folder mode")
        @ValueSwitchWidget
        @Layout(SettingsSection.SingleFileSelection.class)
        @ChoicesProvider(FilterModeChoicesProvider.class)
    ...
    private static final class FilterModeReference implements ParameterReference<SingleFileSelectionMode> {
    }

    private static class FilterModeChoicesProvider implements EnumChoicesProvider<SingleFileSelectionMode> {
        @Override
        public List<SingleFileSelectionMode> choices(final NodeParametersInput context) {
            return List.of(SingleFileSelectionMode.FILE, SingleFileSelectionMode.FOLDER);
        }
    }

    private static final class IsFileSelection implements EffectPredicateProvider {
        @Override
        public EffectPredicate init(final PredicateInitializer i) {
            return i.getEnum(FilterModeReference.class).isOneOf(SingleFileSelectionMode.FILE);
        }
    }

    private static class CompressionChoicesProvider implements StringChoicesProvider {
        @Override
        public List<String> choices(final NodeParametersInput context) {
            return List.of("UNCOMPRESSED", "SNAPPY", "GZIP", "ZSTD");
        }
    }
    ...
    private static final class OutputFileModification implements LegacyFileWriterWithOverwritePolicyOptions.Modifier {
        ...
        public void modify(final Modification.WidgetGroupModifier group) {
            ...
            fileSelection.removeAnnotation(FileWriterWidget.class);
            ...
            fileSelection.modifyAnnotation(Widget.class).withProperty("title", "Output location")
                .withProperty("description", "Select a file system and location where you want to store the file. "
                    + "<br/><b>Mode:</b> Choose between writing to a single file or splitting the data into multiple "
                    + "files in a folder. When writing to a folder, the data is split into files of the "
                    + "specified size with the configured prefix.")
                .modify();
