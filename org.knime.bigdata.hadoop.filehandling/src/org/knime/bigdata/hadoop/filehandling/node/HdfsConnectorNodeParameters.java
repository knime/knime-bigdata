...
class HdfsConnectorNodeParameters implements NodeParameters {
    ...
    void validateOnConfigure() throws InvalidSettingsException {
        ...
        if (m_authentication.m_type == AuthenticationParameters.AuthenticationMethod.SIMPLE
            && StringUtils.isBlank(m_authentication.m_simpleAuth.m_username)) {

            throw new InvalidSettingsException("Username required for simple authentication.");
        }

        // working directory validation
        if (StringUtils.isBlank(m_workingDirectory)
            || !m_workingDirectory.startsWith(HdfsFileSystem.PATH_SEPARATOR)) {
