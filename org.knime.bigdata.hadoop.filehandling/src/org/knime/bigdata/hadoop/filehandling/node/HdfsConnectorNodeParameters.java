...
class HdfsConnectorNodeParameters implements NodeParameters {
    ...
    enum AuthenticationType {
        ...
        AuthType toAuthType() {
            switch (this) {
                case KERBEROS:
                    return HdfsAuth.KERBEROS;
                case SIMPLE:
                default:
                    return HdfsAuth.SIMPLE;
            }
        }
    }
    ...
    static final class AuthenticationFields implements NodeParameters {
        ...
        @Widget(title = "Authentication method", description = "Authentication method to use for connecting to HDFS.")
    ...
    static final class FileSystemConnectionProvider implements StateProvider<FSConnectionProvider> {
        ...
        public FSConnectionProvider computeState(final NodeParametersInput parametersInput)
            ...
            return () -> { // NOSONAR: Longer lambda acceptable, as it improves readability
                final var settings = new HdfsConnectorNodeSettings( //
                    m_protocolSupplier.get().toHdfsProtocol(), //
                    m_hostSupplier.get(), //
                    m_useCustomPortSupplier.get(), //
                    m_customPortSupplier.get(), //
                    m_authTypeSupplier.get().toAuthType(), //
                    m_usernameSupplier.get(), //
                    m_workingDirectorySupplier.get());
                settings.validateValues();

                try {
                    return new HdfsFSConnection(settings.toFSConnectionConfig());
