...
/**
 * Utility class to generate an estimation of the YARN cluster resources that will be requested by a given configuration
 * of the "Create Spark Context (Livy)" node. This class assumed that the YARN-cluster deploy mode is used.
 * 
...
public class SparkResourceEstimator {
    ...
    private static final String EXECUTOR_MEMORY_OVERHEAD_KEY = "spark.yarn.executor.memoryOverhead";

    private static final int EXECUTOR_MEMORY_OVERHEAD_MIN = 384;

    private static final String DRIVER_MEMORY_OVERHEAD_KEY = "spark.yarn.driver.memoryOverhead";
    ...
    private String createDynamicAllocationSummary() {
        ...
        final MemoryUnit containerDisplayMemoryUnit =
            chooseConvenientMemoryUnit(Math.max(driverMemoryMb, executorMemoryMb));
        final MemoryUnit totalDisplayMemoryUnit =
            chooseConvenientMemoryUnit(Math.max(totalMemoryMinMb, totalMemoryMaxMb));
    ...
    private static final NumberFormat MEMORY_NUMBER_FORMAT = new DecimalFormat("##.##");
    ...
    private int estimateExecutorMemoryMb() {
        ...
        final int executorMemoryMb =
            (int)executorResources.getMemoryUnit().convertTo(executorResources.getMemory(), MemoryUnit.MB);
    ...
    private static int estimateYarnMemory(int baseMemoryMb, double overheadFraction, int overheadMinMb) {
