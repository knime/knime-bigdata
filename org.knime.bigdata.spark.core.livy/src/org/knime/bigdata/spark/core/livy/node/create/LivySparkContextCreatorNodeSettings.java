...
public class LivySparkContextCreatorNodeSettings {
    ...
    public enum ExecutorAllocation implements ButtonGroupEnumInterface {
        ...
        @Override
        public boolean isDefault() { return this == FIXED; }
    }
    ...
    private final SettingsModelString m_sparkVersion = new SettingsModelString("sparkVersion", LivySparkContextProvider.HIGHEST_SUPPORTED_SPARK_VERSION.getLabel());
	...
	private final SettingsModelString m_executorAllocation = new SettingsModelString("executorAllocation",
        ExecutorAllocation.FIXED.getActionCommand());
	
	private final SettingsModelIntegerBounded m_fixedExecutors = new SettingsModelIntegerBounded("fixedExecutors", 1, 1, 1000000);
	
	private final SettingsModelIntegerBounded m_dynamicExecutorsMin = new SettingsModelIntegerBounded("dynamicExecutorsMin", 1, 1, 1000000);
	
	private final SettingsModelIntegerBounded m_dynamicExecutorsMax = new SettingsModelIntegerBounded("dynamicExecutorsMax", 10, 1, 1000000);
    ...
    private final SettingsModelIntegerBounded m_connectTimeout = new SettingsModelIntegerBounded("connectTimeout", 30, 0, Integer.MAX_VALUE);
    
    private final SettingsModelIntegerBounded m_responseTimeout = new SettingsModelIntegerBounded("requestTimeout", 30, 0, Integer.MAX_VALUE);
    
    private final SettingsModelIntegerBounded m_jobCheckFrequency = new SettingsModelIntegerBounded("jobCheckFrequency", 1, 1, Integer.MAX_VALUE);
    ...
    public LivySparkContextCreatorNodeSettings() {
        ...
        try {
            ...
            m_customSparkSettings = new SettingsModelKeyValue("customSparkSettings", filterSupportedSparkSettings((List<KeyDescriptor>) supportedSettings));
        } catch (IOException e) {
    ...
    private static List<KeyDescriptor> filterSupportedSparkSettings(List<KeyDescriptor> supportedSettings) {
        return supportedSettings
                .stream()
                .filter((setting) -> !SPARK_SETTINGS_BLACKLIST.contains(setting.getKey()))
    ...
    /**
     * 
     * @return settings model for the number of executors to request when using the "fixed" executor allocation strategy.
     */
    ...
    /**
     * 
     * @return settings model for the min number of executors to request when using the "dynamic" executor allocation strategy.
     */
    ...
    /**
     * 
     * @return settings model for the max number of executors to request when using the "dynamic" executor allocation strategy.
     */
	...
	public LivySparkContextConfig createContextConfig(final SparkContextID contextId) {
		...
		return new LivySparkContextConfig(getSparkVersion(),
				getLivyUrl(),
				getAuthenticationType(),
				getConnectTimeout(),
				getResponseTimeout(),
				getJobCheckFrequency(),
				sparkSettings,
				contextId);
	}
    ...
    private Map<String, String> generateSparkSettings() {
        ...
        if (m_executorResources.overrideDefault()) {
            toReturn.put("spark.executor.memory",
                String.format("%d%s", m_executorResources.getMemory(), m_executorResources.getMemoryUnit().getSparkSettingsUnit()));
