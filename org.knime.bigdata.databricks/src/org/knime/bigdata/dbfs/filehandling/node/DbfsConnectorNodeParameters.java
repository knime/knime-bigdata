...
class DbfsConnectorNodeParameters implements NodeParameters {
    ...
    static final class FileSystemConnectionProvider implements StateProvider<FSConnectionProvider> {
        ...
        private static DbfsFSConnectionConfig createWithWorkspaceConnection(
            ...
            params.m_workingDirectory = DbfsFileSystem.PATH_SEPARATOR;
        ...
        static final class AuthenticationMethodPersistor implements NodeParametersPersistor<AuthenticationMethod> {
            ...
            public AuthenticationMethod load(final NodeSettingsRO settings) throws InvalidSettingsException {
                ...
                throw new InvalidSettingsException(
                    String.format("Unknown authentication method: '%s'. Possible values: '%s', '%s', '%s', '%s'",
                        typeString, CFG_KEY_TOKEN, CFG_KEY_TOKEN_V2, CFG_KEY_USER_PWD, CFG_KEY_USER_PWD_V2));
            }
    ...
    void validateOnConfigure(final CredentialsProvider credProv) throws InvalidSettingsException {
        ...
        if (m_authentication.m_type == AuthenticationMethod.TOKEN) {
            ...
            if (StringUtils.isBlank(cred.getPassword())) {
                throw new InvalidSettingsException("Token required for authentication.");
            }
            ...
            if (StringUtils.isBlank(cred.getUsername())) {
                throw new InvalidSettingsException("Username required for authentication.");
            }
            if (StringUtils.isBlank(cred.getPassword())) {
                throw new InvalidSettingsException("Password required for authentication.");
            }
        }
    ...
    private void validateUrl() throws InvalidSettingsException {
        try {
            final URI uri = new URI(m_url);
            ...
            throw new InvalidSettingsException(String.format("Invalid Databricks deployment URL: %s", e.getMessage()),
                e);
        }
    }

    private void validateWorkingDirectory() throws InvalidSettingsException {
        if (StringUtils.isBlank(m_workingDirectory) || !m_workingDirectory.startsWith(DbfsFileSystem.PATH_SEPARATOR)) {
