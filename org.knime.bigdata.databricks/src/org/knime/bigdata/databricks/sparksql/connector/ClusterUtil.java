...
final class ClusterUtil {
    ...
    static void ensureClusterRunning(final ClusterAPI clusterAPI, final LibrariesAPI librariesAPI,
        ...
        ClusterInfo clusterInfo = clusterAPI.getCluster(clusterId);
    ...
    private static void removeJobJar(final LibrariesAPI librariesAPI, final String clusterId) throws IOException {
        final Optional<LibraryStatus> jobJarStatus = findJobJarOnCluster(librariesAPI, clusterId);
     ...
     * cluster.
     *
     * @return the matching {@link LibraryStatus} if one was found
     */
    private static Optional<LibraryStatus> findJobJarOnCluster(final LibrariesAPI librariesAPI, final String clusterId)
        throws IOException {

        try {
            final ClusterLibraryStatus clusterStatus = librariesAPI.getClusterStatus(clusterId);
            if (clusterStatus.library_statuses != null) {
                for (final LibraryStatus libStatus : clusterStatus.library_statuses) {
                    if (libStatus.library != null && //
                        libStatus.library.jar != null //
                        && libStatus.library.jar.endsWith(DatabricksSparkContext.JOB_JAR_FILENAME)) {
