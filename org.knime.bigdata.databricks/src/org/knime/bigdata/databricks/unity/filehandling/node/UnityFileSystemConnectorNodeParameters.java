...
public class UnityFileSystemConnectorNodeParameters implements NodeParameters {
    ...
    String m_workingDirectory = UnityFileSystem.PATH_SEPARATOR;
    ...
    /**
     * Provides a {@link FSConnectionProvider} based on the Databricks Unity File System connection settings. This
     * enables the working directory field to have a file system browser.
     */
    static final class FileSystemConnectionProvider implements StateProvider<FSConnectionProvider> {
        ...
        private UnityFSConnection createConnection(final PortObjectSpec[] inputSpecs)
            ...
            final UnityFSConnectionConfig config =
                UnityFileSystemConnectorNodeModel.createFSConnectionConfig(credential, spec, workingDir);

            final UnityFSConnection connection = new UnityFSConnection(config); // NOSONAR Java version has no var
            try {
                UnityFileSystemConnectorNodeModel.testConnection(connection);
    ...
    private static void validateWorkingDirectory(final String workingDirectory) throws InvalidSettingsException {
        ...
        } else if (!workingDirectory.startsWith(UnityFileSystem.PATH_SEPARATOR)) {
