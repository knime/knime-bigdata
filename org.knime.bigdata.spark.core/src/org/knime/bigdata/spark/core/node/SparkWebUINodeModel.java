...
/**
 * Basic class that all WebUINodeModel classes need to extend if they work with Spark data/model/etc objects.
 *
 * This class is similar to the {@link SparkNodeModel}, but extends a {@link WebUINodeModel} instead {@link NodeModel}.
 *
 ...
 */
public abstract class SparkWebUINodeModel<S extends NodeParameters> extends WebUINodeModel<S> {

    private static final NodeLogger LOGGER = NodeLogger.getLogger(SparkWebUINodeModel.class);
     ...
     */
    protected SparkWebUINodeModel(final PortType[] inPortTypes, final PortType[] outPortTypes,
        final Class<S> modelSettingsClass) {
        this(inPortTypes, outPortTypes, modelSettingsClass, true);
    }
     ...
     */
    protected SparkWebUINodeModel(final PortType[] inPortTypes, final PortType[] outPortTypes,
        final Class<S> modelSettingsClass, final boolean deleteOnReset) {
        super(inPortTypes, outPortTypes, modelSettingsClass);
     ...
     */
    @Override
    protected final PortObjectSpec[] configure(final PortObjectSpec[] inSpecs, final S modelSettings)
        throws InvalidSettingsException {
        return configureInternal(inSpecs, modelSettings);
    }
     ...
     * @see #configure(PortObjectSpec[], NodeParameters)
     */
    protected abstract PortObjectSpec[] configureInternal(PortObjectSpec[] inSpecs, final S modelSettings)
        throws InvalidSettingsException;
     ...
     */
    @Override
    protected final PortObject[] execute(final PortObject[] inData, final ExecutionContext exec, final S modelSettings)
        throws Exception {

        exec.setMessage("Validate input data...");
        try {
            exec.setMessage("Starting node execution...");
            final PortObject[] portObjects = executeInternal(inData, exec, modelSettings);
     ...
     */
    protected abstract PortObject[] executeInternal(PortObject[] inData, ExecutionContext exec, final S modelSettings)
        throws Exception;
    ...
    protected final void onDispose() {
        LOGGER.debug("In onDispose() of SparkWebUINodeModel. Calling deleteSparkDataObjects.");
    ...
    protected final void reset() {
        if (m_deleteOnReset) {
            LOGGER.debug("In reset() of SparkWebUINodeModel. Calling deleteSparkDataObjects.");
