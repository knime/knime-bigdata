 ...
 */
public class PatternAggregationFunctionTableModel
    extends AbstractAggregationTableModel<SparkSQLAggregationFunction, PatternAggregationFunctionRow> {
     ...
     * Constructor.
     */
    public PatternAggregationFunctionTableModel() {
        this(null);
    }
    /**Constructor for class AggregationColumnTableModel.
     * @param provider {@link AggregationFunctionProvider}
     */
    public PatternAggregationFunctionTableModel(final AggregationFunctionProvider<SparkSQLAggregationFunction> provider) {
        super(new String[] {"Search pattern (double click to change)", "RegEx", "Aggregation (click to change)"},
            new Class<?>[] {PatternAggregationFunctionRow.class, Boolean.class,
            PatternAggregationFunctionRow.class}, false, provider);
    }
    ...
    protected Object getValueAtRow(final int rowIdx, final int columnIndex) {
        final PatternAggregationFunctionRow row = getRow(rowIdx);
        switch (columnIndex) {
            case 0:
                return row;
            case 1:
    ...
    private void updatePattern(final int rowIdx, final String inputPattern) {
        final PatternAggregationFunctionRow old = getRow(rowIdx);
    ...
    private void updateFunction(final int row, final AggregationFunction function) {
        final PatternAggregationFunctionRow old = getRow(row);
    ...
    private void updateRow(final int row, final String pattern, final boolean isRegex,
      ...
      //create a new operator each time it is updated to guarantee that
        ...
        final SparkSQLAggregationFunction methodClone = getAggregationFunctionProvider().getFunction(functionId);
        final PatternAggregationFunctionRow regexAggregator =
                new PatternAggregationFunctionRow(pattern, isRegex, methodClone);
    ...
    public boolean isCellEditable(final int row, final int columnIdx) {
        final PatternAggregationFunctionRow operator = getRow(row);
        if (!operator.isValid()) {
    ...
    protected void toggleMissingCellOption(final int row) {
        final AggregationFunctionRow<SparkSQLAggregationFunction> funcRow = getRow(row);
