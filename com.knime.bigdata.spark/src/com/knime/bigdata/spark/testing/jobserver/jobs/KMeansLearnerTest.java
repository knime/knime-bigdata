...
public class KMeansLearnerTest {
    ...
    private static String getParams(final String aInputDataPath, final Integer aNoOfCluster,
        ...
        return KMeansTask.kmeansLearnerDef(aInputDataPath, new Integer[]{0, 1, 2}, aNumIterations, aNoOfCluster,
            aOutputTableName);
    }
    ...
    @Test
    public void jobValidationShouldCheckIncorrectNumIterationsParameter() throws Throwable {
        String params = getParams("xx", 99, -1, "~spark/data/spark");
        String msg = "Input parameter '" + ParameterConstants.PARAM_NUM_ITERATIONS + "' must be a positive number.";
        myCheck(params, msg);
    ...
    public void runningKMeansDirectlyShouldProduceResult() throws Throwable {
        ...
        try {
            ImportKNIMETableJobTest.importTestTable(contextName, ImportKNIMETableJobTest.TEST_TABLE, "tabKmeans");

            String params = KMeansTask.kmeansLearnerDef("tabKmeans", new Integer[]{0, 2}, 3, 2, "tabKmeansOut");

            String jobId =
                JobControler.startJob(contextName, KMeansLearner.class.getCanonicalName(), params.toString());
    ...
    private static void checkResult(final KNIMESparkContext aContextName, final String aResTable) throws Exception {
        ...
        assertEquals("fetcher should return correct number of rows", ImportKNIMETableJobTest.TEST_TABLE.length, arrayRes.length);
