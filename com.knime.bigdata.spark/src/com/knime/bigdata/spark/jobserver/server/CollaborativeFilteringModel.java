...
public class CollaborativeFilteringModel implements Serializable {
    ...
    /**
     * @param aJob - required for access to named rdds
     * @return user features RDD
     */
    public RDD<Tuple2<Object, double[]>> userFeatures(final KnimeSparkJob aJob) {
        final RDD<Tuple2<Object, double[]>> tmp = aJob.getNamedRdd(m_userFeatures);
        return tmp;
    }

    /**
     * @param aJob - required for access to named rdds
     * @return product features RDD
     */
    public RDD<Tuple2<Object, double[]>> productFeatures(final KnimeSparkJob aJob) {
        final RDD<Tuple2<Object, double[]>> tmp = aJob.getNamedRdd(m_productFeatures);
        return tmp;
    }
