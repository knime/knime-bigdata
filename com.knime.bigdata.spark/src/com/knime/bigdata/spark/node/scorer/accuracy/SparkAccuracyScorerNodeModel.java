...
public class SparkAccuracyScorerNodeModel extends SparkNodeModel {
    ...
    private SparkAccuracyScorerViewData m_viewData;
    ...
    protected PortObject[] executeInternal(final PortObject[] inData, final ExecutionContext exec) throws Exception {
        ...
        final ScorerData result =
            (ScorerData)new ScorerTask(inPort.getData(), tableSpec.findColumnIndex(m_firstCompareColumn),
                tableSpec.findColumnIndex(m_secondCompareColumn), true).execute(exec);

        m_viewData = createViewData(result, tableSpec);
    ...
    private SparkAccuracyScorerViewData createViewData(final ScorerData result, final DataTableSpec inTableSpec) {
        // sort target values
        final Object[] sortedTargetValues = result.getTargetValues().clone();
        Arrays.sort(sortedTargetValues, createComparator(inTableSpec));

        // sort scorer counts accordingly
        final int[][] scorerCountSorted = getSortedScorerCounts(result, sortedTargetValues);

        return new SparkAccuracyScorerViewData(scorerCountSorted, (int)result.getNrRows(), // TODO: the int conversion here might be a problem for very large Spark RDDs.
            result.getFalseCount(), result.getCorrectCount(), m_firstCompareColumn, m_secondCompareColumn,
            toStringArray(sortedTargetValues));
    }

    private int[][] getSortedScorerCounts(final ScorerData result, final Object[] sortedTargetValues) {
        // memorize target value indices
        final Object[] origTargetValues = result.getTargetValues();
        ...
        final int[][] scorerCount = result.getScorerCount();
    ...
    private Comparator<Object> createNumericalSortingComparator() {
        ...
        return new Comparator<Object>() {
            ...
            public int compare(final Object o1, final Object o2) {
                return sign * Double.compare((double)o1, (double)o2);
            }
        };
    }
    ...
    private BufferedDataTable createConfusionMatrixTable(final SparkAccuracyScorerViewData viewData,
    ...
    private BufferedDataTable createAccuracyStatisticsTable(final SparkAccuracyScorerViewData viewData,
        final ExecutionContext exec) {
