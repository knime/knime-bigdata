...
public class FrequentItemSetModel implements Serializable  {
    ...
    FrequentItemSetModel(final String uid, final String frequentItemsObjectName, final double minSupport, final int numPartitions) {
        m_uid = uid;
    ...
    static FrequentItemSetModel fromJobInput(final String uid, final FrequentItemSetJobInput input) {
        if (input.hasNumPartitions()) {
            return new FrequentItemSetModel(uid, input.getFreqItemsOutputObject(), input.getMinSupport(), input.getNumPartitions());
        } else {
            return new FrequentItemSetModel(uid, input.getFreqItemsOutputObject(), input.getMinSupport(), -1);
        }
    }
